...
PRIVATE_KEY=$($HOME/go/bin/0gchaind keys unsafe-export-eth-key wallet --keyring-backend test)
ADDRES=$(echo "0x$(0gchaind debug addr $(0gchaind keys show wallet -a --keyring-backend test) | grep hex | awk '{print $3}')")
#Получаем IP
